...
end_time = time.time()
st.write('before for: ', end_time - start_time)

p.line(circle_x, circle_y, color="#1f77b4", line_width=3, line_alpha=0.6)

start_time = time.time()
for i in range(5000):
  p.line(X, Y, color="#ff7f0e", line_width=5, line_alpha=0.6)
end_time = time.time()
st.write('after for: ', end_time - start_time)

start_time = time.time()
st.bokeh_chart(p,use_container_width=True)
end_time = time.time()
st.write('after print: ', end_time - start_time)
